...
import SymbolSearch from '@/presentation/components/SymbolSearch'
import ChartSettings, {
  ChartSettings as ChartSettingsType,
} from '@/presentation/components/chart/ChartSettings'
import UserDropdown from '@/presentation/components/UserDropdown'
import IndicatorsDropdown from '@/presentation/components/chart/IndicatorsDropdown'
import SaveChartModal from '@/presentation/components/chart/SaveChartModal'
...
const ChartHeader: React.FC<ChartHeaderProps> = ({
  ...
  const fetchExistingCharts = async () => {
    try {
      const response = await api.charts.getCharts({
  ...
  const handleSaveChartSubmit = async (data: {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Optionally refresh existing charts list
        fetchExistingCharts()
      } else {
  ...
  return (
    <>
      ...
      <SaveChartModal
        ...
        onSave={handleSaveChartSubmit}
        defaultValues={{
          symbol: currentSymbol,
          timeframe: selectedTimeframe,
          chartType: chartType,
          indicators: JSON.stringify(chartSettings?.indicators || {}),
          drawingTools: JSON.stringify([]), // TODO: Get actual drawing tools data
          chartSettings: JSON.stringify({
            showVolume: chartSettings?.showVolume || true,
            showGridlines: chartSettings?.showGridlines || true,
            showPeriodHigh: chartSettings?.showPeriodHigh || true,
            showPeriodLow: chartSettings?.showPeriodLow || true,
            periodWeeks: chartSettings?.periodWeeks || 52,
