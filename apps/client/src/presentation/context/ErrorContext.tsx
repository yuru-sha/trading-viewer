...
} from '@/infrastructure/utils/errorRecovery'
  ...
  const showSuccess = useCallback(
    (message: string, title: string = '成功') => {
      return addError({
        ...
        source: 'client',
        autoHide: true,
        duration: 2000, // SUCCESS_NOTIFICATION timeout
      })
    },
    [addError]
  )
  ...
  const showWarning = useCallback(
    (message: string, title: string = '警告') => {
      return addError({
        ...
        source: 'client',
        autoHide: true,
        duration: 4000, // WARNING_NOTIFICATION timeout
      })
    },
    [addError]
  )
      ...
      if (httpError?.response?.status) {
        ...
        switch (status) {
          ...
          case 403:
            // CSRF エラーの場合
            if (
              httpError.response.data?.message?.includes('CSRF') ||
              httpError.response.data?.error?.includes('CSRF')
            ) {
              title = 'セキュリティエラー'
