...
export interface ChartData {
  ...
  volumes: { value: number; itemStyle: { color: string } }[]
}
...
export const useChartData = (
  ...
  const chartData = useMemo((): ChartData => {
    ...
    const volumes = data.map((item, index) => {
      ...
      const color =
        index > 0 && prevItem?.close != null && prevItem.close <= item.close ? '#10b981' : '#ef4444'
      return {
        value: item.volume || 0,
        itemStyle: { color },
      }
  ...
  useEffect(() => {
    ...
    const change =
      latestData?.close && previousData?.close ? latestData.close - previousData.close : 0
    const changePercent =
      latestData?.close && previousData?.close ? (change / previousData.close) * 100 : 0
    ...
    if (latestData) {
      setPriceStats({
        ...
        change,
        changePercent,
        periodHigh: periodHigh ?? visibleHigh,
        periodLow: periodLow ?? visibleLow,
      })
    }
  }, [data, periodWeeks])
