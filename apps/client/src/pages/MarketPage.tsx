...
type MarketTab = 'indices' | 'stocks-japan' | 'stocks-world' | 'crypto' | 'futures' | 'fx' | 'bonds' | 'etf' | 'economics'
...
const MarketPage: React.FC = () => {
  ...
  const [dataCache, setDataCache] = useState<Record<MarketTab, { data: MarketData[], timestamp: number }>>({} as any)
  ...
  const getSymbolsForTab = (tab: MarketTab): string[] => {
    switch (tab) {
      ...
      case 'stocks-japan':
        ...
        return [
          '7203.T', '6758.T', '9984.T', '7974.T', '9433.T', '8306.T', // Toyota, Sony, SoftBank, Nintendo, KDDI, MUFG
          '6861.T', '6902.T', '4063.T', '9432.T', '8035.T', '7267.T', // Keyence, Denso, Shin-Etsu, NTT, Tokyo Electron, Honda
          '6501.T', '8058.T', '8001.T', '9020.T', '7751.T', '4519.T'  // Hitachi, Mitsubishi, Itochu, JR East, Canon, Chugai
        ]
      case 'stocks-world':
        // More world stocks for better categorization
        return [
          'AAPL', 'MSFT', 'GOOGL', 'TSLA', 'AMZN', 'NVDA', // Original
          'META', 'BRK.B', 'JPM', 'V', 'JNJ', 'WMT',       // Meta, Berkshire, JPMorgan, Visa, J&J, Walmart
          'UNH', 'PG', 'DIS', 'MA', 'HD', 'NFLX'           // UnitedHealth, P&G, Disney, Mastercard, Home Depot, Netflix
        ]
      case 'crypto':
      ...
      case 'fx':
        ...
        return [
          'JPY=X', 'EURUSD=X', 'GBPUSD=X', 'AUDUSD=X', 'NZDUSD=X', 'USDCAD=X', // Major pairs
          'USDCHF=X', 'EURGBP=X', 'EURJPY=X', 'GBPJPY=X', 'AUDJPY=X', 'CADJPY=X'  // Cross pairs
        ]
      case 'bonds':
  ...
  useEffect(() => {
    if (lastQuote && 'symbol' in lastQuote && 'data' in lastQuote && typeof lastQuote.data === 'object' && lastQuote.data !== null) {
      const data = lastQuote.data as { price?: number; change?: number; changePercent?: number }
      if (typeof data.price === 'number' && typeof data.change === 'number' && typeof data.changePercent === 'number') {
        setMarketData(prev => 
          prev.map(item => 
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        {loading ? (
          <div className='flex justify-center items-center py-20'>
            <Loading size='lg' text={`Loading ${marketTabs.find(t => t.id === activeTab)?.label} data...`} />
          </div>
        ) : (
          <div>
              {activeTab === 'economics' ? (
                ...
                <div className='bg-white dark:bg-gray-800 rounded-lg shadow p-6'>
                  <h2 className='text-xl font-bold text-gray-900 dark:text-white mb-6'>Economic Indicators</h2>
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    <div className='space-y-4'>
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        <span className='font-medium text-gray-900 dark:text-white'>Japan GDP</span>
                        <span className='text-lg font-bold text-gray-900 dark:text-white'>¥4.03T</span>
                      </div>
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        <span className='font-medium text-gray-900 dark:text-white'>US GDP</span>
                        <span className='text-lg font-bold text-gray-900 dark:text-white'>$29.18T</span>
                      </div>
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        <span className='font-medium text-gray-900 dark:text-white'>Japan Interest Rate</span>
                        <span className='text-lg font-bold text-gray-900 dark:text-white'>0.5%</span>
                      </div>
                    </div>
                    <div className='space-y-4'>
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        <span className='font-medium text-gray-900 dark:text-white'>Japan Unemployment Rate</span>
                        <span className='text-lg font-bold text-gray-900 dark:text-white'>2.5%</span>
                      </div>
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        <span className='font-medium text-gray-900 dark:text-white'>Japan Inflation Rate</span>
                      ...
                      <div className='flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                        ...
                        <span className='text-lg font-bold text-gray-900 dark:text-white'>¥147.74</span>
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        <div className='space-y-2'>
                          <div className='flex items-center justify-between'>
                            <span className='text-2xl font-bold text-gray-900 dark:text-white'>
                              {activeTab === 'fx' || activeTab === 'crypto' ? 
                                item.price.toFixed(4) : 
                                (activeTab === 'bonds' ? `${item.price.toFixed(3)}%` : 
                                `$${item.price.toFixed(2)}`)}
                            </span>
                            <span className={`text-sm font-medium ${
                              item.change >= 0 
                              ...
                              {item.change >= 0 ? '+' : ''}{
                                activeTab === 'fx' || activeTab === 'crypto' ? 
                                  ...
                                  (activeTab === 'bonds' ? `${item.change.toFixed(3)}%` : 
                                  `$${item.change.toFixed(2)}`)}
                            </span>
                          </div>
                          
                          <div className='flex justify-between text-xs text-gray-500'>
                            <span>H: {activeTab === 'fx' || activeTab === 'crypto' ? 
                              item.high.toFixed(4) : 
                              (activeTab === 'bonds' ? `${item.high.toFixed(3)}%` : 
                              `$${item.high.toFixed(2)}`)}</span>
                            <span>L: {activeTab === 'fx' || activeTab === 'crypto' ? 
                              item.low.toFixed(4) : 
                              (activeTab === 'bonds' ? `${item.low.toFixed(3)}%` : 
        ...
        {activeTab !== 'fx' && (
          <div className='mt-12'>
            <MarketNews 
              category={
                activeTab === 'stocks-japan' ? 'japan' :
                activeTab === 'stocks-world' ? 'world' :
