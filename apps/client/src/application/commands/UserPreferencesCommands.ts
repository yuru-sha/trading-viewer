 ...
 */
interface UserPreferences extends UserPreferencesParams {
  ...
  implements UpdateUserPreferencesCommand
{
  ...
  protected doUndo(): UserPreferencesParams {
  ...
  private validateChartDefaults(chartDefaults: unknown): boolean {
    if (typeof chartDefaults !== 'object' || chartDefaults === null) {
      return false
    }

    const chart = chartDefaults as Record<string, unknown>

    if (chart.timeframe && !this.isValidTimeframe(chart.timeframe as string)) {
      return false
    }

    if (chart.chartType && !['candlestick', 'line', 'area'].includes(chart.chartType as string)) {
      return false
    }

    if (chart.indicators && !Array.isArray(chart.indicators)) {
      return false
    }
  ...
  private validateDrawingDefaults(drawingDefaults: unknown): boolean {
    if (typeof drawingDefaults !== 'object' || drawingDefaults === null) {
      return false
    }

    const drawing = drawingDefaults as Record<string, unknown>

    if (drawing.lineColor && !this.isValidColor(drawing.lineColor as string)) {
      return false
    }

    if (drawing.fillColor && !this.isValidColor(drawing.fillColor as string)) {
      return false
    }

    if (drawing.lineWidth && (typeof drawing.lineWidth !== 'number' || drawing.lineWidth < 1)) {
      return false
    }

    if (drawing.textSize && (typeof drawing.textSize !== 'number' || drawing.textSize < 8)) {
      return false
    }
...
export class ResetUserPreferencesCommand extends BaseCommand<void, Record<string, never>> {
  ...
  protected async doUndo(): Promise<Record<string, never>> {
...
export class ImportUserPreferencesCommand extends BaseCommand<void, { data: string }> {
  ...
  protected async doUndo(): Promise<{ data: string }> {
    if (this.backupPreferences) {
      this.context.updateUserPreferences(this.backupPreferences)
    }
    return this.params
