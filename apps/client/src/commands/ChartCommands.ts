...
interface UpdateChartSettingsCommand {
  ...
  implements UpdateChartSettingsCommand
{
  ...
  async doExecute(): Promise<void> {
    ...
    if (this.params.indicators) {
      ...
      this.params.indicators.forEach((indicatorType: string, index: number) => {
  ...
  protected async doUndo(): Promise<ChartSettingsParams> {
 ...
 */
export class AddIndicatorCommand extends BaseCommand<
  string,
...
> {
  ...
  protected async doUndo(): Promise<{ type: string; params: Record<string, unknown> }> {
 ...
 */
export class RemoveIndicatorCommand extends BaseCommand<void, { id: string }> {
  ...
  protected async doUndo(): Promise<{ id: string }> {
...
export class BatchChartCommand extends BaseCommand<void, { commands: BaseCommand[] }> {
  ...
  protected async doUndo(): Promise<{ commands: BaseCommand[] }> {
    for (let i = this.executedCommands.length - 1; i >= 0; i--) {
