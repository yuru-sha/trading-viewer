...
const MiniChart: React.FC<{ data: number[]; isPositive: boolean; type: ChartType }> = ({ data, isPositive, type }) => {
  const width = 120
  const height = 40
        ...
        <div className="flex-1 min-w-0">
          ...
          {(volume || marketCap) && (
            <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              {marketCap && (
                <div>Cap: {marketCap > 1000000000 ? `${(marketCap / 1000000000).toFixed(1)}B` : `${(marketCap / 1000000).toFixed(1)}M`}</div>
              )}
            </div>
          )}
        </div>
