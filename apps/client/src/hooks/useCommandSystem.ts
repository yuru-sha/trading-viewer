...
export function useCommandSystem(options: UseCommandSystemOptions = {}): UseCommandSystemReturn {
  ...
  const execute = useCallback(async <T>(command: ICommand<T>): Promise<CommandResult<T>> => {
    setIsExecuting(true)
  ...
  const executeSequence = useCallback(async <T>(commands: ICommand<T>[]): Promise<CommandResult<T>[]> => {
    setIsExecuting(true)
  ...
  const executeParallel = useCallback(async <T>(commands: ICommand<T>[]): Promise<CommandResult<T>[]> => {
    setIsExecuting(true)
  ...
  const createCommand = useCallback(<T extends ICommand>(type: T['type'], params: T['params']): T => {
...
export function useChartCommands(chartContext: any) {
  ...
  const updateChartSettings = useCallback((settings: any) => {
    const command = commandSystem.createCommand('UPDATE_CHART_SETTINGS', settings)
    return commandSystem.execute(command)
  }, [commandSystem])

  const addIndicator = useCallback((type: string, params: any) => {
