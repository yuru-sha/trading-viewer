...
const MockUserManagementPage = () => {
  ...
  const handleFilterChange = (key: string, value: string) => {
    ...
    setTimeout(() => {
      ...
      // Apply current filters (including new one)
      ...
      if (currentFilters.search) {
        filteredUsers = filteredUsers.filter(
          user =>
            ...
            (user.name && user.name.toLowerCase().includes(currentFilters.search.toLowerCase()))
        )
      }
      ...
      if (currentFilters.status) {
        filteredUsers = filteredUsers.filter(user =>
          currentFilters.status === 'active' ? user.isActive : !user.isActive
        )
      }
  ...
  // Note: Filtering is now handled in handleFilterChange, no need for additional filtering here
...
describe.skip('User Management Integration Tests', () => {
  ...
  describe('Combined Filtering', () => {
    ...
    it('should combine search with role filter', async () => {
      ...
      // First apply search filter - should match 'user@example.com'
      await fireEvent.change(searchInput, { target: { value: 'user' } })
      await waitFor(() => {
        expect(
          screen.getByText((content, element) => {
            return element?.textContent === 'Users (2)'
          })
        ).toBeInTheDocument() // Both 'user@example.com' and 'inactive@example.com' match the search term 'user'
      })

      // Then apply role filter - should only show users with role 'user'
      await fireEvent.change(roleSelect, { target: { value: 'user' } })
      await waitFor(() => {
        expect(screen.getByText('Users (2)')).toBeInTheDocument() // Both users have role 'user'
        expect(screen.getByText('user@example.com')).toBeInTheDocument()
        expect(screen.getByText('inactive@example.com')).toBeInTheDocument()
      })
    })
  ...
  describe('User Experience', () => {
    ...
    it('should provide visual feedback for user status', () => {
      ...
      expect(userBadges.length).toBe(3) // 2 user role badges + 1 table header
    })
  })
})
