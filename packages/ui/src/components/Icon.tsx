...
import {
  ...
  Plus, Minus, Edit, Trash2, Copy, Download, Upload, RefreshCw,
  BarChart3, TrendingUp, TrendingDown, Target, Bell, BellPlus,
  User, Users, LogIn, LogOut,
  Loader, Loader2, CheckCircle, XCircle, AlertTriangle, Info,
  Eye, EyeOff,
  Play, Pause,
  Settings, HelpCircle, Search, Heart, Database, BookOpen, ExternalLink,
  Sun, Moon,
  // 追加でよく使われるアイコン
  Calendar, Clock, Filter, MoreHorizontal, MoreVertical,
  ArrowUp, ArrowDown, ArrowLeft, ArrowRight,
  Save, Share, Star, Heart as HeartFilled,
  Maximize, Minimize, Zap, Lock, Unlock,
  // チャート関連で不足しているアイコン
  Crosshair, Palette, ChartNoAxesGantt, ChartNoAxesCombined,
  AlignHorizontalDistributeCenter, TrendingUpDown,
  Volume2, Layers, Camera,
  PanelLeftClose, PanelLeftOpen, PanelBottomClose, PanelBottomOpen,
  // その他の不足アイコン
  Smartphone, Monitor, Tablet, Keyboard,
  // 絵文字置き換え用アイコン
  Package, Lightbulb
} from 'lucide-react'
...
export const Icon: React.FC<IconProps> = ({ name, size = 16, className, color }) => {
  ...
  if (!IconComponent) {
    console.warn(`Icon "${iconName}" not found in iconMap. Add it to the import list for tree-shaking optimization.`)
    return null
  }
